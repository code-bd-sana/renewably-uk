        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>Thank you for signing up with <strong>Renewably UK</strong> on behalf of <strong>${companyName}</strong>.</p>
              
              <div class="status-box">
                <p style="margin: 0; font-weight: bold;">Your account is currently under review</p>
              </div>
              
              <p>You will receive a separate email once your account has been:</p>
              <ul>
                <li><strong>Approved</strong> → you can then log in and start using the portal</li>
                <li><strong>Declined</strong> → we will explain the reason</li>
              </ul>
...
Thank you for registering with Renewably UK on behalf of ${companyName}.

Your account is currently under review by our team.

You will be notified by email once a decision has been made:
• Approved → you can log in and use the portal
• Declined → we will provide details
              ...
              <div class="info-box">
                ...
                ${
                  requestedRoles.length > 0
                    ? `<p><strong>Requested Roles:</strong> ${requestedRoles.join(", ")}</p>`
                    : ""
                }
...
export async function sendCertificateEmail(
  ...
  try {
    // console.log(
    //   `📧 Preparing email with ${pdfAttachments.length} attachments for ${toEmail}`,
    // );
    ...
    const mailOptions = {
      ...
      to: toEmail,
      subject: `Your Insurance Certificate${totalProducts > 1 ? "s" : ""} - Policy ${policyNumber}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Your Insurance Certificates</title>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
            .container { max-width: 600px; margin: 0 auto; padding: 20px; }
            .header { background: linear-gradient(135deg, #0F47A8 0%, #1E88E5 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
            ...
            .certificate-list { background: #f0f7ff; padding: 15px; border-radius: 5px; margin: 20px 0; }
            .certificate-item { padding: 8px 0; border-bottom: 1px solid #dbeafe; }
            .certificate-item:last-child { border-bottom: none; }
            .certificate-number { display: inline-block; background: #0F47A8; color: white; width: 24px; height: 24px; border-radius: 50%; text-align: center; line-height: 24px; margin-right: 10px; }
            .footer { text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; color: #666; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0; font-size: 24px;">Your Insurance Certificates</h1>
              <p style="margin: 10px 0 0; opacity: 0.9;">Certificate(s) Attached</p>
            </div>
            <div class="content">
              <p>Dear <strong>${policyHolderName}</strong>,</p>
              
              <p>Your Insurance Backed Guarantee certificates have been generated successfully by <strong>${contractorName}</strong>.</p>
              
              <div class="info-box">
                <p style="margin: 0;"><strong>Policy Number:</strong> ${policyNumber}</p>
                <p style="margin: 5px 0 0;"><strong>Contractor:</strong> ${contractorName}</p>
                <p style="margin: 5px 0 0;"><strong>Total Certificates:</strong> ${totalProducts}</p>
                <p style="margin: 5px 0 0;"><strong>Email Sent To:</strong> ${toEmail}</p>
                <p style="margin: 5px 0 0;"><strong>Date:</strong> ${new Date().toLocaleDateString("en-GB")}</p>
              </div>

              <p>If you have any questions about your certificates, please contact ${contractorName} directly.</p>
...
Your Insurance Backed Guarantee certificates have been generated successfully by ${contractorName}.
...
${
  Array.isArray(pdfAttachments) && pdfAttachments.length > 0
    ? pdfAttachments
        .map(
          (att, index) =>
            `${index + 1}. ${att.filename}${att.productType ? ` (${att.productType})` : ""}`,
        )
        .join("\n")
    : "1. Certificate.pdf (single certificate)"
}

Important Information:
- Each certificate provides insurance-backed guarantee for a specific installation
- Please keep ALL certificate files together with your property documents
- Share these certificates with your solicitor if you sell the property
- Refer to the policy wording for full terms and conditions

Note: You are receiving ${totalProducts} separate PDF files attached to this email, one for each product/measure in your policy.

If you have any questions about your certificates, please contact ${contractorName} directly.

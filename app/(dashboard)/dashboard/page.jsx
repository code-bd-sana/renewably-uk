      ...
      try {
        ...
        if (certsResponse.ok) {
          ...
          if (certsData.success) {
            ...
            // Update stats
            const now = new Date();
            const thisMonthCertificates = certsData.certificates.filter(cert => {
              const certDate = new Date(cert.inceptionDate.split('/').reverse().join('-'));
            ...
            setStats({
              ...
              editPending: certsData.certificates.filter(cert => cert.status === 'pending_edit').length
            });
          }
        }
  ...
  return (
    <main className="p-4 lg:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-2xl font-bold">{stats.thisMonthCertificates}</h3>
            <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center">
              <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        <div className="p-4 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <h3 className="font-semibold text-lg">Recent Insurance Backed Guarantee Certificates</h3>
          <input
            type="text"
                ...
                <div key={index} className="p-4 border-b">
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium">{cert.policyNo}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-semibold">{cert.price}</span>
                    </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Policy No</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Policy Holder Name</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Measure Type</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Contract Value</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Inception Date</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Expiry Date</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Transaction Type</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-600">Price</th>
                ...
                <tbody>
                  {filteredCertificates.map((cert, index) => (
                    <tr key={index} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">{cert.contractValue}</td>
                      <td className="px-4 py-3 text-sm">{cert.inceptionDate}</td>
                      <td className="px-4 py-3 text-sm">{cert.expiryDate}</td>
                      <td className="px-4 py-3 text-sm">{cert.transactionType}</td>
                      <td className="px-4 py-3 text-sm font-semibold">{cert.price}</td>

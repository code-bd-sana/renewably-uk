...
export default function CreateInsuranceForm() {
    ...
    const [selectedTime, setSelectedTime] = useState({ hour: '01', minute: '34', period: 'AM' });
    ...
    const countries = ['United Kingdom', 'United States', 'Canada', 'Australia', 'Germany', 'France'];
    const dropdownOptions = ['Not Required', 'Assigned', 'Pending Assignment', 'External Provider'];
    ...
    const fetchContractorData = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (data.user) {  // Changed from data.contractor to data.user
                ...
                {monthNames.map((month, index) => (
                    <button
                        ...
                        onClick={() => {
                            ...
                            const apiDate = `${year}-${String(monthIndex).padStart(2, '0')}-${String(expiryDate.getDate()).padStart(2, '0')}`;
                        ...
                        className={`p-3 rounded-lg text-sm font-medium transition-colors ${selectedMonth === `${month} ${currentMonth.getFullYear()}`
            ...
            <div className="grid grid-cols-7 gap-2">
                {getDaysInMonth(currentMonth).map((dayObj, index) => (
                    <button
                        ...
                        className={`aspect-square flex items-center justify-center text-sm rounded-lg transition-colors ${!dayObj.isCurrentMonth
                            ? 'text-gray-300'
                            : selectedDate === `${dayObj.day}/${currentMonth.getMonth() + 1}/${currentMonth.getFullYear()}`
                                ? 'bg-blue-600 text-white'
                                : 'hover:bg-gray-100'
                            }`}
                    >
            ...
            <div className="absolute z-50 top-full mt-2 bg-white rounded-lg shadow-lg border w-full max-w-md">
                {options.map((option, index) => (
                    <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center justify-between ${index === 0 ? 'rounded-t-lg' : ''
                            } ${index === options.length - 1 ? 'rounded-b-lg' : 'border-b'}`}
                    >
                        <span className={index === 0 ? 'font-medium' : ''}>{option}</span>
                        {selected === option && (
                            <Check size={18} className="text-white bg-blue-600 rounded-full p-0.5" />
                        )}
                    </button>
                ))}
            </div>
        ...
        try {
            ...
            formData.products.forEach((product, index) => {
                ...
                if (isNaN(contractValue) || contractValue <= 0) {
                    validationErrors.push(`Product ${index + 1}: Please enter a valid contract value (must be a number greater than 0)`);
                }

                // Check total project cost if provided
                if (product.totalProjectCost && product.totalProjectCost.trim() !== '') {
                    const totalCost = parseFloat(product.totalProjectCost);
                    if (isNaN(totalCost) || totalCost < 0) {
                        validationErrors.push(`Product ${index + 1}: Please enter a valid total project cost`);
                    }
                }

                // Check dates
                if (!product.inceptionDate) {
                    validationErrors.push(`Product ${index + 1}: Inception date is required`);
                }
    ...
    const contractValue = parseFloat(product.contractValue);
    const totalProjectCost = product.totalProjectCost && product.totalProjectCost.trim() !== ''
        ? parseFloat(product.totalProjectCost)
        : 0;

    // Calculate price (5% of contract value or use existing price if provided)
    const price = product.price ? parseFloat(product.price) : contractValue * 0.05;
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2">Contractor Office Address</label>
                            <input
                                type="text"
                    ...
                    {formData.products.map((product, index) => (
                        <div key={product.id} className="mb-6 p-4 border rounded-lg">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-2">Cover Option</label>
                                    <input
                                        type="text"
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        value={product.inceptionDate ?
                                            `${new Date(product.inceptionDate).getDate()}/${new Date(product.inceptionDate).getMonth() + 1}/${new Date(product.inceptionDate).getFullYear()}`
                                            : ''
                                        }
                                        onClick={() => {
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => updateProduct(product.id, 'contractValue', e.target.value)}
                                        required
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => updateProduct(product.id, 'totalProjectCost', e.target.value)}
                                    />
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                    <h2 className="text-lg font-semibold mb-4">Compliance and Submission</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="relative dropdown-container">
                            <label className="block text-sm font-medium mb-2">Retrofit Assessor</label>
                            <button
                                type="button"
                            ...
                            <DropdownMenu
                                ...
                                onSelect={(option) => setFormData({ ...formData, retrofitAssessor: option })}
                                show={showRetrofitAssessor}
                                onClose={() => setShowRetrofitAssessor(false)}
                            />
                        </div>
                        <div className="relative dropdown-container">
                            <label className="block text-sm font-medium mb-2">Retrofit Coordinator</label>
                            <button
                                type="button"
                                onClick={() => setShowRetrofitCoordinator(!showRetrofitCoordinator)}
                                className="w-full border rounded-lg px-3 py-2 text-left flex items-center justify-between"
                            >
                                <span>{formData.retrofitCoordinator || 'Select your cover option'}</span>
                                <ChevronRight size={18} />
                            </button>
                            <DropdownMenu
                                options={dropdownOptions}
                                selected={formData.retrofitCoordinator}
                                onSelect={(option) => setFormData({ ...formData, retrofitCoordinator: option })}
                                show={showRetrofitCoordinator}
                                onClose={() => setShowRetrofitCoordinator(false)}
                            />
                        </div>
                        <div className="relative dropdown-container">
                            <label className="block text-sm font-medium mb-2">Funding Partner</label>
                            <button
                                type="button"
                            ...
                            <DropdownMenu
                                ...
                                onSelect={(option) => setFormData({ ...formData, fundingPartner: option })}
                                show={showFundingPartner}
                                onClose={() => setShowFundingPartner(false)}
                            />
                        </div>
                        <div className="relative dropdown-container">
                            <label className="block text-sm font-medium mb-2">Scheme Provider</label>
                            <button
                                type="button"
                                onClick={() => setShowSchemeProvider(!showSchemeProvider)}
                                className="w-full border rounded-lg px-3 py-2 text-left flex items-center justify-between"
                            >
                                <span>{formData.schemeProvider || 'Select your cover option'}</span>
                                <ChevronRight size={18} />
                            </button>
                            <DropdownMenu
                                options={dropdownOptions}
                                selected={formData.schemeProvider}
                                onSelect={(option) => setFormData({ ...formData, schemeProvider: option })}
